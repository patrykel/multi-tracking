...
class RecoToCSVAnalyzer : public edm::EDAnalyzer {
   public:
      explicit RecoToCSVAnalyzer(const edm::ParameterSet&);
      ~RecoToCSVAnalyzer();
   ...
   private:
      ...
      void hits_log(std::vector<RPRecoHit> &hits, int eventId, unsigned int line_no, std::string &prefix);
      void lines_log(std::vector<RPRecognizedPatterns::Line> &lines, std::string &prefix, int eventId);

      ofstream eventsCSV;  // to create csv file for event dump
};
